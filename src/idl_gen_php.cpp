...
namespace flatbuffers {
	namespace php {
		...
		static void NewRootTypeFromBuffer(const StructDef &struct_def,
			...
			code += Indent + Indent + "return ($obj->Init($bb->GetInt($bb->GetPosition()) + $bb->GetPosition(), $bb));\n";
			code += Indent + "}\n\n";
		}
		...
		static void GetVectorLen(const StructDef &struct_def,
			...
			code += Indent + Indent + "return $o != 0 ? $this->__vector_len($o) : 0;\n";
			code += Indent + "}\n\n";
		}
		...
		static void GetScalarFieldOfStruct(const StructDef &struct_def,
			...
			code += Indent + " */\n";
			code += Indent + "public function " + getter + MakeCamel(field.name) + "()\n";
			code += Indent + "{\n";
			code += Indent + Indent + "return ";

			code += "$this->bb->Get" + MakeCamel(GenTypeGet(field.value.type)) + "($this->bb_pos + ";
		...
		static void GetStructFieldOfStruct(const StructDef &struct_def,
			...
			code += Indent + "public function Get";
			code += MakeCamel(field.name) + "()\n";
			//code += GenTypeGet(field.value.type) + " $obj)\n";
			code += Indent + "{\n";
		...
		static void GetStructFieldOfTable(const StructDef &struct_def,
			...
			code += MakeCamel(field.name);
			code += "(){\n";
			code += Indent + Indent + "$obj = new " + MakeCamel(GenTypeGet(field.value.type)) + "();\n";
			code += Indent + Indent +
				"$o = $this->__offset(" +
				NumToString(field.value.offset) +
				");\n";
			code += Indent + Indent + "return $o != 0 ? $obj->init($o + $this->bb_pos, $this->bb) : " + GenDefaultValue(field.value) + ";\n";
		...
		static void GetStringField(const StructDef &struct_def,
			...
			code += Indent + Indent +
				...
				NumToString(field.value.offset) +
				");\n";
			code += Indent + Indent + "return $o != 0 ? $this->__string($o + $this->bb_pos) : " + GenDefaultValue(field.value) + ";\n";
			code += Indent + "}\n\n";
		}
		...
		static void GetUnionField(const StructDef &struct_def,
			...
			code += Indent + Indent +
				...
				NumToString(field.value.offset) +
				");\n";
			code += Indent + Indent + "return $o != 0 ? $this->__union($obj, $o) : null;\n";
			code += Indent + "}\n\n";
		}
		...
		static void GetMemberOfVectorOfStruct(const StructDef &struct_def,
			...
			case BASE_TYPE_STRUCT:
				if (struct_def.fixed) {
					code += Indent + Indent + "return $o != 0 ? $obj->init($this->bb_pos +" 
			...
			case BASE_TYPE_VECTOR:
				code += "// base_type_vector\n";
				if (vectortype.base_type == BASE_TYPE_STRUCT)
				{
					code += Indent + Indent + "return $o != 0 ? $obj->Init(";
					if (vectortype.struct_def->fixed)
					{
						code += "$this->__vector($o) + $j *" + NumToString(InlineSize(vectortype));
					} else
					{
						code += "$this->__indirect($this->__vector($o) + $j * " + NumToString(InlineSize(vectortype)) + ")";
					}
					code += ", $this->bb) : null;\n";
				}
				break;
			case BASE_TYPE_UNION:
				code += Indent + Indent + "return $o != 0 ? $this->" + GenGetter(field.value.type) + "($obj, $o); null;\n";
				break;
			}
		...
		static void GetMemberOfVectorOfNonStruct(const StructDef &struct_def,
			...
			{
				code += Indent + Indent + "return $o != 0 ? $this->__string($this->__vector($o) + $j * " + NumToString(InlineSize(vectortype)) + ") : " + GenDefaultValue(field.value) + ";\n";

			} else
			{
				code += Indent + Indent + "return $o != 0 ? $this->bb->Get" + MakeCamel(GenTypeGet(field.value.type)) + "($this->__vector($o) + $j * " + NumToString(InlineSize(vectortype)) + ") : " + GenDefaultValue(field.value) + ";\n";
			}
			code += Indent + "}\n\n";
		}
		...
		static void StructBuilderBody(const StructDef &struct_def,
			...
			std::string *code_ptr) {
			std::string &code = *code_ptr;
			code += Indent + Indent + "$builder->prep(" + NumToString(struct_def.minalign) + ", ";
			...
			it != struct_def.fields.vec.rend();
				...
				auto &field = **it;
				if (field.padding)
					code += Indent + Indent + "$builder->pad(" + NumToString(field.padding) + ");\n";
				if (IsStruct(field.value.type)) {
		...
		static void GetStartOfTable(const StructDef &struct_def,
			...
			it != struct_def.fields.vec.end();
				...
				code += Indent + Indent + "self::Add" + MakeCamel(field.name) + "($builder, $" + field.name + ");\n";
			}
		...
		static void BuildFieldOfTable(const StructDef &struct_def,
			...
			code += Indent + " * @return void\n";
			code += Indent + " */\n";
			code += Indent + "public static function " + "Add" + MakeCamel(field.name);
			code += "(FlatBufferBuilder $builder, ";
			code += "$" + MakeCamel(field.name, false);
			code += "){\n ";
		...
		static void BuildVectorOfTable(const StructDef &struct_def,
			...
			code += ");\n";
			code += Indent + Indent + "for ($i = count($data) - 1; $i >= 0; $i--) {\n";
			if (IsScalar(field.value.type.VectorType().base_type))
			{
				code += Indent + Indent + Indent + "$builder->Add" + MakeCamel(GenTypeBasic(field.value.type.VectorType())) +"($data[$i]);\n";
			} else
			{
				code += Indent + Indent + Indent + "$builder->AddOffset($data[$i]);\n";
			}
			code += Indent + Indent + "}\n";
		...
		static void GetEndOffsetOnTable(const Parser &parser, const StructDef &struct_def,
			std::string *code_ptr) {
			std::string &code = *code_ptr;
			...
			if (parser.root_struct_def_ == &struct_def) {
				...
				code += Indent + "public static function Finish" + struct_def.name + "Buffer(FlatBufferBuilder $builder, $offset)\n";
				code += Indent + "{\n";
		...
		static void GenTableBuilders(const Parser &parser, const StructDef &struct_def,
			std::string *code_ptr) {
			...
			it != struct_def.fields.vec.end();
				...
				{
					std::string &code = *code_ptr;
					code += Indent + "public static function Add" +  MakeCamel(field.name) +"(FlatBufferBuilder $builder, $offset) {\n";
					code += Indent + Indent + "$builder->AddOffsetX(" + NumToString(offset)  + ", $offset, 0);\n";
					code += Indent + "}\n\n";
				} else
				{
		...
		static void GenStruct(const Parser &parser, const StructDef &struct_def,
			...
			if (!struct_def.fixed)
			{
				if (parser.file_identifier_.length()) {
					...
					code += "Identifier()\n";
					code += Indent + "{\n";
					code += Indent + Indent + "return \"" + parser.file_identifier_ + "\";\n";
					code += Indent + "}\n\n";
		...
		static void GenEnum(const EnumDef &enum_def, std::string *code_ptr) {
			...
			code += Indent + Indent + Indent + "throw new \\Exception('not found');\n";
			code += Indent + Indent + "}\n";
...
#undef FLATBUFFERS_TD
	...
	bool GeneratePhp(const Parser &parser,
		...
		it != parser.structs_.vec.end(); ++it) {
			std::string declcode;
			php::GenStruct(parser, **it, &declcode, parser.root_struct_def_);
